...
const BannerCategoryId = ({bannerLists}) => {
    ...
    function addBasketHandler(productDetail) {
        //bedenler json'a eklendiğinde her bedenin productId si farklı olduğu için seçili olan bedenin verisi gönderilecek.
        dispatch(addBasket({
            id: productDetail.productId,
            url: productDetail.url,
            name: productDetail.text,
                                    ...
                                    <Link href={`/detail/${x.productId}`}>
                                        <a>
                                            <img className={styles.categoryImg}
                                                 src={Array.isArray(x.url) ? x.url[0] : x.url}/>
                                        </a>
                                    </Link>
                                </div>

                                <div className={styles.sizes}>
                                    <button className={styles.sizesTitle} onClick={() => addBasketHandler(x)}>Beden Seç
                                    </button>
